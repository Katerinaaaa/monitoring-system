...
int PongController::passEnter(QString passwd)
{
...
QString PongController::strPolitic(QString politic, QString ip) {
...
QString PongController::strData(QString message, QString ip) {
    QByteArray data; // Массив данных для отправки
...
void PongController::read() {
    while (socket->hasPendingDatagrams()) {
        ...
        cout << "Computers on the network: " << line << endl;
        file.close();
        ...
        if (peremennaya == 0){
            ...
            cout << "Such a delegate already exist. Sending a signal to qml." << endl;
...
QString PongController::readSnap(QString str){
    ...
    //qDebug() << "Получил Снап";
    cout << "Snapshot read." << endl;

    return snapshot;
}

QString PongController::readKeylog(QString str){
    int x = 0;
    int y = 0;
...
QString PongController::wantKeylog(QString addr){
...
QString PongController::saveToLogFile(QString str) {
    QDate now = QDate::currentDate();
    QString dateStr = now.toString("dd-MM-yyyy");
    qDebug() << dateStr;
...
QString PongController::showPolitic(QString filename){
